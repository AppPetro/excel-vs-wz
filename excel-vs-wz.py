...
if ext == "pdf":
    with pdfplumber.open(uploaded_wz) as pdf:
        def parse_wz_table(df_table: pd.DataFrame):
            ...
            # === TYLKO TUTAJ ZMIANA: usuwamy split()[-1], by nie tracić tys. ===
            if not col_ean or not col_qty:
                # fallback dla broken-header (np. Termin ważności Ilość)
                for c in cols:
            ...
            for _, row in df_table.iterrows():
                ...
                if not re.fullmatch(r"\d{13}", raw_ean):
                    continue
                # zamiast split() bierzemy całość, usuwamy spacery i zmieniamy przecinek
    ...
    for _, r in df_wz_raw.iterrows():
        ...
        if not re.fullmatch(r"\d{13}", raw_ean):
            continue
        raw_qty = str(r[col_qty_wz]).strip().replace(" ", "").replace(",", ".")
        try:
            qty = float(raw_qty)
        except:
            qty = 0.0
